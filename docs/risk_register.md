...
| R017 | q3 自由终端时间和零推力模型可能产生非运营解 | 中 | 高 | 最优解大幅延长 `tf` 或频繁贴近 `T=0` | q3-T08 已报告 `tf/t_base=1.01887` 和近零推力比例 `0`；新增 `N=121` 怠速推力局部敏感性，三档怠速边界均未激活；后续仍需 `N=241` 加强和基础油耗项扩展 | q3 | open |
...
| R030 | q3 Gate 2 高度敏感性被误用为最终燃油高度敏感性 | 中 | 高 | 直接引用 `optimized_hmax_sensitivity.csv` 解释最终省油策略对 `h_max` 的敏感性 | 文档明确该表只支持 Gate 2 Stage 1 可行性；已新增 `no_wind_final_hmax_sensitivity.csv` 作为最终燃油目标下的 `N=121` 局部重优化敏感性，但完整论文结论仍需 `N=241` 或更高迭代加强 | q3 | open |
