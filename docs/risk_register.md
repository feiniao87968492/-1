...
| R030 | q3 Gate 2 高度敏感性被误用为最终燃油高度敏感性 | 中 | 高 | 直接引用 `optimized_hmax_sensitivity.csv` 解释最终省油策略对 `h_max` 的敏感性 | 文档明确该表只支持 Gate 2 Stage 1 可行性；最终无风燃油优化完成后需重跑 `h_max in {10950,11500,12000,12500} m` 的最优燃油敏感性 | q3 | open |
| R031 | q3 最终燃油优化只报告终端质量而缺少验收诊断 | 中 | 高 | 后续脚本只输出 `m_f`、`J` 或轨迹表，未报告重积分、燃油恒等式、目标网格收敛、多初值、时间比、近零推力和 KKT/最优性 | review14 后在 `approach.md`、`results.md` 和证据链中固定验收表；未满足阈值不得登记 `q3-T07`/`q3-T08` 为正式最优结果 | q3 | open |
| R032 | q3 `N=241` 最终燃油全量 NLP 求解时间不可接受 | 高 | 高 | 全变量 SLSQP 在 `N=61`/`N=241` 最终燃油目标下超时，无法完成验收 | 当前仅输出 Gate 2 可行候选和失败验证表；后续需改用稀疏 NLP、低维连续射击、CasADi/pyomo 或分层控制参数化 | q3 | open |
