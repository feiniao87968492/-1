...
针对民用客机巡航阶段燃油消耗集中、飞行高度与速度相互制约且高空风场不可忽略的特点，本文围绕高度、速度、质量、阻力、油耗和风场之间的耦合关系，建立"经验策略基线--固定路径油耗积分--无风最优控制--有风综合比较"的递进模型。模型以点质量动力学、升阻平衡和燃油率方程为基础，并在不同子问题中分别固定终止质量、共同航程或终端机械状态，使策略比较、油耗复算和优化建模具有一致的物理口径。
...
\textbf{对于问题三，}将经验策略推广为固定航程、固定终端高度和速度、终端质量自由的最优控制问题，以最大化终端质量等价刻画最小油耗，避免"固定终止质量再最小化油耗"的目标退化。先用航程域直接配点构造连续可行初值，再以 降维控制打靶法 进行最终燃油优化。无风优化在 $241$ 节点网格上得到总油耗 $10342.814\ \mathrm{kg}$、终端质量 $62107.186\ \mathrm{kg}$、终端时间 $805.679\ \mathrm{s}$，相对可行初值 $10450.000\ \mathrm{kg}$ 降低约 $107.186\ \mathrm{kg}$。连续验证表明终端速度重积分误差 $1.71\times10^{-5}\ \mathrm{m/s}$、燃油恒等式残差 $8.58\times10^{-5}\ \mathrm{kg}$，无风最终燃油优化已形成可复核结果。

\textbf{对于问题四，}在问题三无风最优基础上引入有风航程域最优控制，采用 降维控制打靶法 局部优化。固定共同航程 $189781.310\ \mathrm{m}$ 下，有风局部优化轨迹总油耗 $9863.840\ \mathrm{kg}$，比等速基线节省 $563.416\ \mathrm{kg}$（$5.403\%$）；固定燃油 $10450\ \mathrm{kg}$ 口径下可达航程下界估计 $201168.189\ \mathrm{m}$，较共同航程增加 约 $6.0\%$。速度偏离惩罚系数 $\beta$ 从标称值降至 $0.8$ 倍时局部可行解燃油减少 $14.116\ \mathrm{kg}$，增至 $1.2$ 倍时增加 $6.230\ \mathrm{kg}$。

上述"经验策略基线--路径油耗积分--无风最优控制--有风综合比较"四层递进框架表明，高度、速度、推力与风场的协同调整是降低油耗的关键；模型经步长稳定性、网格收敛、连续重积分和灵敏度分析检验，具有较好的物理一致性和数值可靠性。
...
燃油消耗率采用题设的速度偏离惩罚形式：
\begin{equation}
\frac{dm}{dt}=-c_FT\left[1+\beta(V-V_{\mathrm{opt}})^2\right].
\label{eq:fuel}
\end{equation}
...
燃油流率仍采用
\begin{equation}
q_f(x)=c_F T_{\mathrm{req}}(x)\left[1+\beta(V_{\mathrm{ref}}-V_{\mathrm{opt}})^2\right].
...
第二问解决的是固定飞机怎么飞、研究不同大气条件下油耗变化。第三问进一步追问：\textbf{飞机自己应该怎么飞，才能让固定航程的油耗最低？} 此时高度 $h(t)$、速度 $V(t)$、推力 $T(t)$ 和航迹角 $\gamma(t)$ 不再由经验策略预设，而成为算法自主搜索的未知曲线。前三问的任务层级对比如表~\ref{tab:q3_task_comparison}。

\begin{table}[H]
\centering\caption{前三问研究任务层级对比}
\label{tab:q3_task_comparison}
\begin{tabular}{cll}\toprule
问序 & 剖面来源 & 核心任务 \\ \midrule
Q1 & 给定（经验策略） & 给定高度速度，算轨迹与油耗 \\
Q2 & 给定（复用 Q1 路径） & 固定轨迹，换天气，看油耗变化 \\
Q3 & 待优化未知曲线 & 轨迹自由，让算法找最省油路线 \\ \bottomrule
\end{tabular}
\end{table}

Q1 和 Q2 的共同特征是飞行轨迹完全确定，求解方法为常微分方程数值积分或路径积分。Q3 取消了高度和速度的固定约束，飞机成为随时间变化的动态系统，数学本质从正向仿真转为\textbf{带约束的最优控制问题}\cite{kuang2012,chang2023}。由于该问题含非线性气动阻力、质量变化和路径约束，本文综合运用最优控制建模、航程域直接配点和连续验证三层手段求解\cite{jia2024,optimal_control2017}。有风条件下的最优控制结果在问题四中给出。

\subsection{最优控制建模}

\subsubsection{状态量与控制量}

系统状态取 $z=(x,h,V,m)$，分别为已飞行水平距离、实时飞行高度、真空速和飞机瞬时总质量；控制输入取 $u=(T,\gamma)$，分别为发动机推力和航迹爬升角。
...
\end{equation}
即最小化总燃油消耗。\textbf{这里最关键的建模考量是：不可直接固定终点质量 $m(t_f)$。} 若错误地固定 $m(t_f)=62000\ \mathrm{kg}$，则
\[
J=72450-62000=10450\ \mathrm{kg}
\]
已成为常数，优化目标失去区分意义，无论算法如何调整轨迹都无法进一步降低 $J$。因此本文仅约束航程、始末高度和速度，将 $m(t_f)$ 作为自由优化变量，通过\textbf{最大化落地剩余质量等价于最小化总燃油消耗}。同时，固定终端高度和速度可防止优化器通过降低终点机械能换取表观省油。
...
\dot m = -c_F T\left[1+\beta\left(V-V_{\mathrm{opt}}\right)^2\right],
\end{cases}
\label{eq:q3_dynamics}
\end{equation}
其中 $W(h)$ 为高度相关风速（无风时 $W=0$），$c_F$ 为推力燃油消耗系数，$\beta$ 为速度偏离惩罚系数，$V_{\mathrm{opt}}$ 为最佳效率速度，$\Phi(V)=1+\beta(V-V_{\mathrm{opt}})^2$ 为速度偏离惩罚函数。

\subsubsection{气动力计算}

巡航过程中升力近似平衡重力的法向分量。由 $L=\frac12\rho(h)V^2SC_L=mg\cos\gamma$ 解出升力系数
\begin{equation}
C_L = \frac{2mg\cos\gamma}{\rho(h)V^2 S},
\label{eq:q3_CL}
\end{equation}
阻力系数采用抛物线极曲线 $C_D=C_{D0}+kC_L^2$，阻力为
\begin{equation}
D = \frac12\rho(h)V^2 S \left(C_{D0} + k C_L^2\right).
\label{eq:q3_drag}
\end{equation}
物理耦合逻辑为：高度变化引起密度 $\rho(h)$ 变化 $\rightarrow$ 升力系数与阻力同步改变 $\rightarrow$ 推力需求与燃油消耗联动变化。

\subsubsection{初始条件、终端条件与飞行限制}
...
\end{equation}
终点质量仅设下限，不设上界，保留为优化自由变量。飞行包线约束为
\begin{equation}
...
\caption{问题三：最优巡航控制建模流程图}
\label{fig:q3model}
\end{figure}

\subsection{求解思路：航程域变换}

\subsubsection{庞特里亚金极大值原理}

根据庞特里亚金极大值原理\cite{pontryagin}，构造哈密顿量
\begin{equation}
H=\lambda_x(V\cos\gamma+W(h))+\lambda_hV\sin\gamma+\lambda_V\frac{T-D-mg\sin\gamma}{m}
-\lambda_m c_FT\left[1+\beta(V-V_{\mathrm{opt}})^2\right],
\label{eq:q3_hamiltonian}
\end{equation}
其中 $\lambda_x,\lambda_h,\lambda_V,\lambda_m$ 为协态变量。最优控制的一阶必要条件为 $\partial H/\partial T=0$、$\partial H/\partial\gamma=0$，可推导出两点边值问题。但协态变量初值完全未知，数值求解对初值高度敏感，工程上极少直接用于轨迹寻优\cite{optimal_control2017}。因此本文仅将庞特里亚金极大值原理必要条件用于最优性结构解释和后续诊断，不作为主求解器。

\subsubsection{航程离散与配点}

本文主数值方案为航程域直接配点法。其核心变换是将时间域 $t\in[0,t_f]$（终端时间 $t_f$ 未知）转换为以航程 $x$ 为独立自变量的固定区间 $x\in[0,X_f]$，从而避免未知终端时间带来的求解困难。航程域下，时间域动力学 \eqref{eq:q3_dynamics} 除以地速 $V_g=V\cos\gamma+W(h)$ 得到
\begin{align}
\frac{dh}{dx} &= \frac{V\sin\gamma}{V_g},\\
\frac{dm}{dx} &= -\frac{c_F T\,\Phi(V)}{V_g},\\
\frac{dt}{dx} &= \frac{1}{V_g}.
\end{align}
将航程区间离散为 $N$ 个配点，连续未知轨迹 $h(x),V(x),T(x),\gamma(x)$ 转化为各节点处的有限维离散变量，通过梯形配点约束保证离散点满足航程域微分方程，使无限维最优控制问题转化为有限维非线性规划\cite{jia2024}。
...
\textbf{第一阶段：可行性求解。} 读取 Q1--Q2 的飞机气动、燃油与大气模型参数，将航程 $x\in[0,X_f]$ 离散为 $N=241$ 个配点，以各节点处的 $h,V,m,T,\gamma$ 为优化变量，加入动力学配点约束、飞行包线上下限和始末边界条件。此阶段不追求节油，优化目标仅为最小化终端质量松弛 $s$（即令终点质量尽量接近 $62000\ \mathrm{kg}$）。当 $s$ 降至数值零后，用独立微分方程重积分检查连续轨迹是否满足全部约束，得到一条连续可行轨迹作为第二阶段的初始猜测。大气模型采用 $C^1$ 温度平滑和静力压力积分，避免轨迹穿越 $11\ \mathrm{km}$ 层界时出现不可微问题。

\textbf{第二阶段：最终燃油优化。} 以可行性求解结果为初始猜测，采用降维控制打靶法——将推力和航迹角用少量控制结点参数化，沿航程积分原连续动力学，把终端高度、速度、质量下限和路径约束作为优化约束，直接最小化 $J=m_0-m(t_f)$。该方法减少了直接配点中的大规模离散变量，使最终优化更接近连续动力学口径。在 $61\to121\to241$ 节点逐级加密中验证网格稳定性，最终采用 $N=241$ 节点结果。

为避免直接把有风基线误当作无风可行初值，在固定等速几何路径上分别重算配置风场和无风场景。结果见表 \ref{tab:q3_baseline_feasibility}。
...
表~\ref{tab:q3_baseline_feasibility} 表明，配置风场下固定路径终点质量为 $62022.744\ \mathrm{kg}$，勉强满足约束；无风条件下同一路径终点质量仅为 $61163.474\ \mathrm{kg}$，质量缺口约 $836.526\ \mathrm{kg}$。图~\ref{fig:q3_feasibility_bridge} 将这一差异直观呈现：无风固定路径明显落到质量下限以下，而重新优化后终端质量恢复到 $62107.186\ \mathrm{kg}$。
...
因此，问题三的直接配点并非单纯追求节油，而是\textbf{先解决无风条件下的可行性}。$N=241$ 配点可行性通过，意味着在该分辨率下存在一条满足所有动力学约束、飞行包线约束和始末边界约束的连续轨迹，这是后续最终燃油优化的物理基础。

表~\ref{tab:q3_mesh_convergence} 给出三个网格层级下的最终优化结果：节点数由 $61$ 增至 $241$ 时，总油耗始终稳定在 $10342.81\ \mathrm{kg}$ 附近，$N=121$ 与 $N=241$ 的差异仅为 $3.28\times10^{-4}\ \mathrm{kg}$，说明最终结果不是某一离散网格的偶然输出。
...
图~\ref{fig:q3_grid_stability} 进一步显示三层网格的总油耗均稳定在 $10342.81\ \mathrm{kg}$ 附近，验证了网格无关性。
...
最终轨迹的主要形态见图~\ref{fig:q3_final_trajectory}。高度曲线先快速爬升，随后在巡航中段贴近 $12000\ \mathrm{m}$ 上界；质量曲线随航程单调下降，并在终点保留约 $107.186\ \mathrm{kg}$ 的质量余量。这一形态说明，高度上界在无风最终优化中接近活跃，终端质量约束则未被压到边界。
...
从数值上看，可行性候选燃油为 $10450.000\ \mathrm{kg}$，最终优化解降至 $10342.814\ \mathrm{kg}$，减少约 $107.186\ \mathrm{kg}$；由于终端时间比 $t_f/t_{\mathrm{base}}=1.01887$，该收益并非来自明显拉长飞行时间。表~\ref{tab:q3_final_optimal} 给出无风最终优化的核心状态和约束余量。
...
\textbf{对最优轨迹的物理理解：} 无风最优轨迹的核心策略是\textbf{尽快爬升到高度上界附近巡航}。高度越高，空气密度越低，在相同升力平衡下所需升力系数更大但阻力更小（因为动压降低）；同时高空飞行使速度偏离惩罚项中的 $V$ 更接近 $V_{\mathrm{opt}}$，降低了单位推力燃油消耗率。终端阶段则通过调整推力和速度满足固定终点高度和速度约束。这一"先高后稳"的形态与传统巡航优化中"飞高省油"的工程经验一致。

\subsection{解的验证与敏感性}

为确认最终优化不是离散网格上的偶然结果，本文对最终优化解进行独立连续验证。验证内容包括原微分方程重积分、燃油恒等式、网格目标差异、多初值一致性、时间约束和连续路径约束。结果见表~\ref{tab:q3_final_validation}。
...
由表~\ref{tab:q3_final_validation} 和图~\ref{fig:q3_validation_summary} 可见，所有关键残差均低于阈值，且验证结果全部通过。其中终端速度重积分误差仅为 $1.71\times10^{-5}\ \mathrm{m/s}$，燃油恒等式残差为 $8.58\times10^{-5}\ \mathrm{kg}$，说明无风最终燃油优化结果具有连续动力学一致性。

局部灵敏度方面，本文还给出了高度上界和怠速推力的初步重优化结果。$h_{\max}=12500\ \mathrm{m}$ 的局部计算得到燃油约 $10334.057\ \mathrm{kg}$，比 $12000\ \mathrm{m}$ 上界情形低约 $8.757\ \mathrm{kg}$，提示高度上界可能是影响无风最优油耗的活跃约束；但该灵敏度当前基于 $N=121$ 局部重优化，且部分状态未收敛，因此只能作为趋势提示。怠速推力扰动的油耗变化不超过 $0.01\ \mathrm{kg}$，与最终轨迹最小推力约 $19.634\ \mathrm{kN}$、高于怠速下界的事实一致；但同样不宜写成全局结论。
...
有风最优控制以问题三无风最终燃油优化轨迹作为初始猜测，采用航程域 降维控制打靶法：将推力和航迹角用 5 个分段线性控制结点参数化，状态由原连续动力学沿航程积分，终端高度、速度和质量下限作为优化约束。该方法降低了直接配点中的大规模离散变量，使最终优化更接近连续动力学口径。优化采用序列二次规划方法，最大迭代 300 次，输出节点 41 个。
...
$\beta$ 灵敏度采用逐场景重新求解方式，每个扰动因子下均从标称解出发进行有风 降维控制打靶法 重优化，而非在标称轨迹上后验重算。表 \ref{tab:q4_beta_sensitivity} 给出五档结果。
...
问题三和问题四的动力学方程中，推力 $T$ 既驱动运动方程又直接进入燃油率 $\dot m=-c_FT\,\Phi(V)$，隐含了指令推力等于有效推力且单位燃油消耗率不随飞行条件变化的假设。实际涡扇发动机的安装推力受进气道总压恢复和喷管流场损失影响，有效推力通常低于指令值。引入安装效率 $\eta_{\mathrm{inst}}(h,M,T_{\mathrm{cmd}})$ 和燃油乘子 $\phi_{\mathrm{inst}}(h,M)$ 后，
\begin{equation}
...
问题三最终轨迹的最大高度达到 $11999.960\ \mathrm{m}$，贴近 $12000\ \mathrm{m}$ 高度上界，因此高度上界是无风最优结果中需要重点关注的约束。局部重优化显示，当 $h_{\max}$ 放宽到 $12500\ \mathrm{m}$ 时，燃油约为 $10334.057\ \mathrm{kg}$，较当前主结果降低约 $8.757\ \mathrm{kg}$；但该结果基于 $N=121$ 局部计算且存在未收敛状态，不宜作为正式节油结论。怠速推力扰动下油耗变化小于 $0.01\ \mathrm{kg}$，与最终解最小推力 $19.634\ \mathrm{kN}$ 明显高于怠速下界一致，可作为"当前无风解未贴近怠速推力边界"的佐证。

\subsection{问题四有风优化的 $\beta$ 灵敏性}

问题四在有风 降维控制打靶法 下对 $\beta$ 进行逐场景重求解灵敏度分析（详见表 \ref{tab:q4_beta_sensitivity}）。$\beta$ 降低 $20\%$ 时局部可行解燃油减少 $14.116\ \mathrm{kg}$（$0.143\%$），增大 $20\%$ 时增加 $6.230\ \mathrm{kg}$（$0.063\%$），整体波动幅度不超过 $0.15\%$。与问题一中 $\beta$ 扰动对经验策略飞行时间约 $1.5\%$--$3\%$ 的影响相比，有风优化轨迹对 $\beta$ 更不敏感，说明优化器通过调整高度和速度剖面吸收了部分速度偏离惩罚的变化。
...
问题三先采用航程域直接配点构造连续可行初值，再用 降维控制打靶法 完成无风最终燃油优化。最终优化在 $N=61,121,241$ 三个网格层级上均收敛，总油耗分别为 $10342.805$、$10342.815$ 和 $10342.814\ \mathrm{kg}$，其中 $N=121$ 与 $N=241$ 的差异仅为 $3.28\times10^{-4}\ \mathrm{kg}$。连续验证进一步表明：终端速度重积分误差 $1.71\times10^{-5}\ \mathrm{m/s}$，终端高度误差 $3.62\times10^{-4}\ \mathrm{m}$，燃油恒等式残差 $8.58\times10^{-5}\ \mathrm{kg}$，连续约束违反为 $0$。因此，无风最终燃油优化结果已具备连续动力学一致性。

\subsection{问题四有风优化检验}

问题四有风 降维控制打靶法 以问题三无风最终轨迹作为初始猜测，在 $41$ 个输出节点和 $5$ 个控制结点下完成局部优化。独立微分方程重积分给出终端高度误差 $6.31\times10^{-7}\ \mathrm{m}$、终端速度误差 $1.51\times10^{-8}\ \mathrm{m/s}$、最大尺度化约束违反 $0$、燃油恒等式残差 $0.0316\ \mathrm{kg}$。这些检验量相对阈值的位置见图 \ref{fig:q4_validation_card}。
...
\begin{enumerate}[left=0pt]
    ...
    \item \textbf{对比口径统一，结果解释客观。} 有风优化基于无风最优初值引入，采用固定航程与固定燃油双口径严密比较差异；结论完全基于当前模型数值证据，不做主观预判。
\end{enumerate}
...
\item \textbf{补充完整最优性诊断。}对有风最优轨迹构造协态和驻值条件，验证庞特里亚金极大值原理必要条件的满足程度。
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{itemize}
    \item 提示词 (Prompt)："请帮助实现问题三的航程域直接配点法数值求解。具体需求如下：(1) 将航程区间 $[0, X_f]$ 离散为 $N=241$ 个配点，以各节点处的 $h, V, m, T, \gamma$ 为优化变量；(2) 加入动力学配点约束、飞行包线上下限和始末边界条件；(3) 第一阶段以最小化终端质量松弛 $s$ 为目标，构造连续可行轨迹；(4) 第二阶段以可行性结果为初始猜测，采用降维控制打靶法最小化总油耗 $J=m_0-m(t_f)$；(5) 使用 scipy.optimize.minimize 的 SLSQP 方法求解非线性规划。"
