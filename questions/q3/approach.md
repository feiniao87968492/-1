...
- 初始网格：可从 `N=31` 开始；Gate 2 可行性验证已完成 `N=31/61/121/241`，最终燃油优化建议按 `N=61 -> 121 -> 241` continuation。
...
| 连续路径约束违反 | `<=1e-6` 无量纲 | 覆盖高度、速度、马赫、推力、航迹角和质量下限 |
| 燃油恒等式残差 | `<=0.05 kg` | 检查 `abs((m0-mf)-integral cT*T*Phi(V)/Vg dx)` |
| 目标网格收敛 | `abs(J_121-J_241)<=1 kg` 且相对变化 `<=1e-4` | 不只检查可行性误差，必须检查最优目标值收敛 |
| 多初值目标差 | `max(J)-min(J)<=1 kg` | 至少比较 Gate 2 初值、平直/等速初值和扰动初值 |
| 近零推力比例 | 报告，不设硬阈值 | 用于识别 `T_min=0` 的零油耗滑翔偏差 |
