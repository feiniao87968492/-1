...
已完成基准 `h_max=12000 m` 的 `N=31/61/121` 网格收敛诊断，但尚未通过连续重积分门槛。后续仍需检查：
...
当前非 dry-run Gate 2 Stage 1 的离散质量松弛已达到数值零，`N=31/61/121` 网格收敛显示重积分误差按约二阶下降，但连续重积分尚未通过，状态仍为 `discrete_feasible_reintegration_failed`。下一步应执行连续重构修正、局部加密或更高阶转录诊断；只有离散可行性、独立 ODE 重积分和网格收敛同时通过，才可进入无风最优轨迹求解。
...
- 本轮没有生成最终最优轨迹或最优油耗；非 dry-run Gate 2 Stage 1 只用于可行性门控。
...
- Gate 1 是低维参数化可行性搜索，不是全局可行性证明；Gate 2 Stage 1 当前状态 `discrete_feasible_reintegration_failed` 不能被写成“连续可行”或“已最优”。
