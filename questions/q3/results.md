...
- 完整 collocation 可行性 Gate 的 `N=61/121` 网格收敛、`h_max` 敏感性连续诊断和 11 km 大气平滑方案敏感性。
...
当前非 dry-run Gate 2 Stage 1 的离散质量松弛已达到数值零，但连续重积分尚未通过，状态为 `discrete_feasible_reintegration_failed`。下一步应执行网格收敛和连续重构诊断；只有离散可行性、独立 ODE 重积分和网格收敛同时通过，才可进入无风最优轨迹求解。
