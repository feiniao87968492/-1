...
尚未运行最终无风燃油最优问题，因此没有燃油最优轨迹的 Hamiltonian 残差数值。已完成固定路径预检查：
...
上述 `h_max` 表只表示 warm-start 诊断，不表示优化后的 `s*(h_max)`。review8 后已另行生成正式优化后的 `optimized_hmax_sensitivity.csv`，并在 review9 后补充每行连续重积分诊断。
