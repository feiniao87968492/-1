...
| q3-E02 | 2026-07-07 | 无风可行性 Gate | `artifacts/q1/data/constant_speed_profile.csv`; `configs/default.yaml` | 航程域参数化可行性搜索，目标为最小终端质量松弛 | `s*`、终端状态误差、最大约束违反、路径残差 | `s*=10.214 kg`，固定路径松弛为 `836.579 kg`；状态为 `needs_relaxation` | 不能进入正式无风最优求解；需扩展可行性 NLP 或调整口径 |
