...
只有当终端质量下限不激活，即 `nu=0` 时，才退化为 `lambda_m(tf)=-1`。因此正式 PMP 诊断不能无条件使用 `-1`，必须报告终端质量不等式是否激活。
...
若后续加入 `tf<=1.05 t_base` 或其他终端时间上限，且该上限激活，则自由终端时间条件也要加入对应终端乘子修正，不能继续直接写成简单的 `H(tf)=0`。
...
高度上界若形成边界弧，例如 `h=h_max`，还需满足切向条件：

```text
dot h = V sin(gamma) = 0
```

在 `V>0` 时边界巡航段至少要求 `gamma=0`。进入点、退出点、路径约束乘子以及伴随量跳跃需要按分段结构处理；当前文档中的互补松弛只足以作为论文概述，不能作为完整连续 PMP 验证。
