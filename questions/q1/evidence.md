...
| Q1-C03 | 等马赫策略最终高度约 `10437.818 m`，用时约 `811.032 s`，航程约 `211331.296 m`，其中空速累计航程约 `193315.658 m`、风场贡献约 `18015.638 m` | `artifacts/q1/data/constant_mach_profile.csv` 与 `strategy_comparison.csv` | `validation_summary.csv` 中初始条件、质量单调、升力和能量残差、马赫约束均通过 | 依赖标准声速模型和同一闭合条件 | supported |
...
- 小航迹角检查通过，最大航迹角为等速 `0.3563 deg`、等马赫 `0.2877 deg`；
- 隐式 ODE 分母正性、推力正性、等速解析高度和等马赫约束检查均通过；
- 有风相对无风增加地面航程；
